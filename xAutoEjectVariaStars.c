...
#include <string.h>
#include <stdbool.h>
#define Radius_bp 0.00833333 //deg, which is about 30 arcsec 
main()
{
        int     i,j,N0,N1,m;
	...
	char    VStype[30];
        ...
        float   y2[20000],y3[20000];
	char    x8[5000][50],x7[5000][50],y1[20000][50],y4[20000][50],y5[20000][50];
        FILE    *fp1; 
//	FILE	*fp1,fp1;
	FILE	*fave;
	char    *first, *second;
...
//		printf("%f %s %s\n",x1[i],x7[i],x8[i]);
	...
	if(fp1)
	{
	while((fscanf(fp1,"%s %f %f %s %s\n",id,&ra,&dec,mag,VStype))!=EOF)
          {
	  sprintf(y1[j],"%s",id);
          y2[j]=ra;
	  y3[j]=dec;
	  sprintf(y4[j],"%s",mag);
	  sprintf(y5[j],"%s",VStype);
          j++;
          }
	  N1=j;
		...
		for(i=0;i<N0;i++)
		{
			...
			for(j=0;j<N1;j++)
			{	
				if(abs(y2[j]-x1[i])>Radius_bp || abs(y3[j]-x2[i])>Radius_bp )
				       m++;
				else
					break;
			}
