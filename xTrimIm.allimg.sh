...
#do
#	imagenewnum=`echo $imagenum | awk '{print($1+$msub)}'`
#	imagenewnumstr=`echo $imagenewnum | awk '{print length($0)}'`
#	if [ $imagenewnumstr -gt 1 ]
#	then
#		imagenewnum=`echo $imagenum | awk '{printf("000",$1-sub)}'`
#	elif	[ $imagenewnumstr -gt 2 ]
#	then
#		 imagenewnum=`echo $imagenum | awk '{printf("00",$1-sub)}'`	
#	elif	 [ $imagenewnumstr -gt 3 ]
#	then
#		 imagenewnum=`echo $imagenum | awk '{printf("0",$1-sub)}'`
#	else
#		:
#	fi
...
ximTrim=`cat new.log | awk '{printf("%.0f",$3)}'`
yimTrim=`cat new.log | awk '{printf("%.0f",$4)}'`
imsubname=`echo $prefix"_OT_X"$ximTrim"Y"$yimTrim".fit"`
refsubname=`echo $prefix"_OT_X"$ximTrim"Y"$yimTrim"_ref.fit"`
if test -r $imsubname
then
	break
fi
imsubnamePNG=`echo $prefix"_OT_X"$ximTrim"Y"$yimTrim".png"`
refsubnamePNG=`echo $prefix"_OT_X"$ximTrim"Y"$yimTrim"_ref.png"`
...
ds9  -fits $imsubname -scale limits -100 200 -zoom to fit -geometry 500x500 -saveimage jpeg 50 $imsubnamePNG -exit 
wait
#echo $refsubname $refsubnamePNG 
ds9  -fits $refsubname -scale limits -100 200 -zoom to fit -geometry 500x500 -saveimage jpeg 50 $refsubnamePNG -exit 
wait
#============================
#mv $imsubname $refsubname $imsubnamePNG $refsubnamePNG $otsubdictory
}
...
for msub in -2 -1 0 1 2
do
        imagenewnum=`echo $imagenum $msub | awk '{print($1+$2)}'`
        imagenewnumstr=`echo $imagenewnum | awk '{print length($0)}'`
