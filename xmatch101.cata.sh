...
#	CoordDiff_table=`echo $FITFILE | sed 's/\.fit/.fit.coordiff.cat/'`
	...
	OTtable2frameHave=`echo $FITFILE | sed 's/\.fit/.fit.skyOT.2frameHave/'`
	OTtable2frameNoAll=`cat $FITFILE | sed 's/\.fit/.fit.skyOT.2frameNotAllHave/'`
...
#	subimage=`echo $OUTPUT_new | sed 's/\.fit.sexnew/.sub.fit/'`
	...
	gifimg=`echo $OUTPUT_new | sed 's/\.fit.sexnew/.gif.fit/'`
	outgif=`echo $OUTPUT_new | sed 's/\.fit.sexnew/.gif/'`

	echo $FITFILE 
	
#	./xdarkcorr.sh $FITFILE $darkname 
	./xdarkflatcoor.sh $FITFILE $darkname $flatname
	sex $FITFILE  -c  xmatchdaofind.sex -DETECT_THRESH 3 -ANALYSIS_THRESH 3 -CATALOG_NAME $OUTPUT -CHECKIMAGE_TYPE BACKGROUND -CHECKIMAGE_NAME $bg 
#=========================================================================
...
#        echo "display(image=\"$tempfile\",frame=2)" >>login.cl #display temp file in frame 2
	echo "tvmark(frame=1,coords=\"$newimageOTxyFis\", outimag=\"$gifimg\", mark=\"circle\",radii=50,color=204,label-,lengths=10)" >>login.cl # tvmark new OT in frame 2
	echo "export(images=\"$gifimg\",binfiles=\"$outgif\",format=\"gif\",header-,outtype=\"i4\" )" >>login.cl
	
#	echo "tvmark(frame=1,coords=\"temp2framecoord_new\",mark=\"circle\",radii=100,color=205,label-)" >>login.cl # tvmark new OT in frame 2
...
#======================================================
	...
	cp $OTtable2frameHave new2frame.obj	
	echo "=============="
	cat new2frame.obj | awk '{print($7,$8,$1,$2)}' 
	echo "=============="
...
#        display -resize 1012x1012+0+0  plot3frame.png &
	...
	cat listskyotfile | tail -100 >listskyotfileHis
