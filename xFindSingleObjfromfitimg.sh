...
then
    ...
    echo "The data is fit image , not fits image"
    echo "output file is in /data/cutfitsfile"
    exit 0
fi
...
echo $fitsforcut
    FILEforsub=$1
    imsubname=`echo $FILEforsub | sed 's/\.fit/_cut.fit/g'`
    xim=$xtempfile
    yim=$ytempfile
...
do
	...
	do
		...
		then
			...
			then
				echo "This image is not the required fit"
			else 
				xcutfits  $fitsforcut
			fi
		fi  
