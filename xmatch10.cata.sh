...
#	subimage=`echo $OUTPUT_new | sed 's/\.fit.sexnew/.sub.fit/'`
	...
	./xdarkflatcorr.sh $FITFILE $darkname $flatname
	date >xtime
...
#=======================================================
	echo `date` "2th match"
        ...
        echo "xyxymatch(\"$OUTPUT_new\",\"$tempmatchstars\", \"mattmp1\",toleranc=25, xcolumn=1,ycolumn=2,xrcolum=1,yrcolum=2,separation=7, matchin=\"$matchflag\", inter-,verbo-) " >>login.cl
...
#with in the information about the newxyshift from the last image to do the xyxymatch
		...
		then
			...
			./xmatchTolerWithshift.sh $FITFILE
		else
			echo "------------NO newxyshift.cat-----------"
			echo "----------Tranangle match for this image------"
			./xmatchTriangle.sh $FITFILE	
			cat allres1 | sort -n -k 7 | head -2000 | awk '{print($1,$2,$3)}' > $OUTPUT_new
			./xmatchTolerWithshift.sh $FITFILE	
...
#======================================================
	...
	echo "=============="
	if test -r new2frame.obj
	then
		cat new2frame.obj | awk '{print($1,$2,$3,$4,$5,$6,$7,$8)}' | column -t 
	fi
	echo "=============="
...
#	echo `date` "Calculating the mean FWHM for this image"
	...
	cat xtime | tr '\n' ' ' >>xtime1
	echo  >>xtime1
	cat xtimeall.cat xtime1 >xtimeall.cat1
	mv xtimeall.cat1 xtimeall.cat
	rm -rf xtime xtime1
